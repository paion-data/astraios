...
If a data store is unable to fully implement filtering, sorting, or pagination, it can instead rely on the Astraios
framework to perform these functions in memory.  By default, however, Astraios pushes these responsibilities to the
store.

Included Stores
---------------

Astraios comes bundled with a number of data stores:
...
6. [Aggregation Data Store](analytics) - A store which provides computation of groupable measures (similar to SQL group
   by). The aggregation store has custom annotations that map an Astraios model to native SQL queries against a JDBC
   database.
