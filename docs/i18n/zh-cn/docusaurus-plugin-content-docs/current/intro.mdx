...
**[JAX-RS]** webservice ONLY backed by its reference implementation [Jetty] running as a WAR inside [Jetty] container.
...
服务，背后采用官方指定实现框架 [Jetty] 来实现，并以 WAR 的形式运行于 [Jetty] 容器服务中。
