...
1. 使用 `git clone https://github.com/paion-data/astraios.git` 克隆存储库，或者
...
当我们运行这个 cURL 调用时，应该会看到返回很多 JSON，这就是我们新插入的对象！
...
<Tabs>
  <TabItem value="browser" label="Web Browser" default>
    ...
    只需打开最喜欢的浏览器并访问 **http://localhost:8080/v1/data/book**
...
请注意，我们创建它时并没有设置新书记录的任何属性。更新数据来帮助用户与添加新数据一样简单。
使用以下 cURL 调用更新我们的模型。
